...
class LunarLanderEnvironment(environment.BaseEnvironment):
    ...
    def env_step(self, action):
        ...
        # your code here
         # Check if lander has landed successfully
        if pos_y == 0:  # Assuming touching the ground
            if -3 <= vel_y <= 3 and -10 <= vel_x <= 10 and (5 <= angle <= 355) and (land_x - 1 <= pos_x <= land_x + 1):
                reward = 100  # Successful landing reward
                terminal = True
            else:
        ...
        if fuel <= 0 and pos_y > 0:  # Out of fuel and not on the ground
            reward -= 50  # Penalty for crashing due to fuel exhaustion
            terminal = True

        # Reward structure for maintaining stability and controlling fuel usage
        reward += -abs(vel_y) - abs(vel_x) - abs(angle - 180) / 10
        if pos_x < land_x - 1 or pos_x > land_x + 1:  # Penalize for being outside landing zone
            reward -= 10
