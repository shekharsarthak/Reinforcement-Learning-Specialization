...
class PendulumTileCoder:
    ...
    def get_tiles(self, angle, ang_vel):
        ...
        # ----------------
        # your code here
         # Normalize and scale the angle to [0, self.num_tiles]
        angle_normalized = (angle + np.pi) / (2 * np.pi)
        angle_scaled = angle_normalized * self.num_tiles

        # Normalize and scale the angular velocity to [0, self.num_tiles]
        ang_vel_normalized = (ang_vel + 2 * np.pi) / (4 * np.pi)
        ang_vel_scaled = ang_vel_normalized * self.num_tiles
        # ----------------
...
def compute_softmax_prob(actor_w, tiles):
    ...
    # ----------------
    # your code here
    state_action_preferences = [actor_w[a][tiles].sum() for a in range(len(actor_w))]
    # ----------------
