...
def get_gradient(s, weights):
    ...
    # your code here
     # Forward pass
    psi = my_matmul(s, weights[0]["W"]) + weights[0]["b"]  # Shape: (1, num_hidden_units)
    x = np.maximum(0, psi)  # Shape: (1, num_hidden_units)

    # Backward pass
    # Indicator function for ReLU derivative
    indicator = (x > 0).astype(float)  # Shape: (1, num_hidden_units)

    # Compute delta
    w1_T = weights[1]["W"].T  # Shape: (1, num_hidden_units)
    delta = w1_T * indicator  # Element-wise multiplication

    # Gradients with respect to W[0] and b[0]
    grad_W0 = my_matmul(s.T, delta)  # Shape: (num_states, num_hidden_units)
    grad_b0 = delta  # Shape: (1, num_hidden_units)

    # Gradients with respect to W[1] and b[1]
    grad_W1 = x.T  # Shape: (num_hidden_units, 1)
    grad_b1 = np.array([[1]])  # Shape: (1, 1)
...
class TDAgent(BaseAgent):
    ...
    def agent_step(self, reward, state):
        ...
        s_tp1 = one_hot(state, self.num_states)
        v_hat_tp1 = get_value(s_tp1, self.weights)

        delta = reward + self.discount_factor * v_hat_tp1 - v_hat_t
        # ----------------
        ...
        # ----------------
        # your code here
        grads = get_gradient(s_t, self.weights)
        # ----------------
