...
class SarsaAgent(BaseAgent):
    ...
    def select_action(self, tiles):
        ...
        for action in range(self.num_actions):
            # Compute Q(s, a) by summing weights for active tiles
            q_value = np.sum(self.w[action][tiles])
    ...
    def agent_step(self, reward, state):
        ...
        # Compute Q(s, a)
        Q_sa = np.sum(self.w[self.last_action][self.previous_tiles])
        # Compute Q(s', a')
        Q_sprime_aprime = action_value
        
        # Compute TD error
        delta = reward + self.gamma * Q_sprime_aprime - Q_sa
        
        # Update weights for the previous action and tiles
        self.w[self.last_action][self.previous_tiles] += self.alpha * delta
        
        # Update last action and tiles
        self.last_action = current_action
        self.previous_tiles = np.copy(active_tiles)
        return self.last_action
        # ----------------
    ...
    def agent_end(self, reward):
        ...
        # Compute TD error (since episode has ended, there's no next Q)
        delta = reward - Q_sa
