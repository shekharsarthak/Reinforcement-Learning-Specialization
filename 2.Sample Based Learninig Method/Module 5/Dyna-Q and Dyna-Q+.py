...
get_ipython().run_cell_magic('add_to', 'DynaQAgent', '\n# -----------\n# Graded Cell\n# -----------\n\ndef update_model(self, past_state, past_action, state, reward):\n    """updates the model \n    \n    Args:\n        past_state       (int): s\n        past_action      (int): a\n        state            (int): s\'\n        reward           (int): r\n    Returns:\n        Nothing\n    """\n    # Update the model with the (s,a,s\',r) tuple (1~4 lines)\n    \n    # ----------------\n    # If the past_state is not in the model, initialize its action dictionary\n    if past_state not in self.model:\n        self.model[past_state] = {}\n    # Store the (next_state, reward) tuple for the given action in the past_state\n    self.model[past_state][past_action] = (state, reward)\n    ')
...
get_ipython().run_cell_magic('add_to', 'DynaQPlusAgent', '\n# -----------\n# Graded Cell\n# -----------\n\ndef planning_step(self):\n    """performs planning, i.e. indirect RL.\n\n    Args:\n        None\n    Returns:\n        Nothing\n    """\n    \n    # The indirect RL step:\n    # - Choose a state and action from the set of experiences that are stored in the model. (~2 lines)\n    # - Query the model with this state-action pair for the predicted next state and reward.(~1 line)\n    # - **Add the bonus to the reward** (~1 line)\n    # - Update the action values with this simulated experience.                            (2~4 lines)\n    # - Repeat for the required number of planning steps.\n    #\n    # Note that the update equation is different for terminal and non-terminal transitions. \n    # To differentiate between a terminal and a non-terminal next state, assume that the model stores\n    # the terminal state as a dummy state like -1\n    #\n    # Important: remember you have a random number generator \'planning_rand_generator\' as \n    #     a part of the class which you need to use as self.planning_rand_generator.choice()\n    #     For the sake of reproducibility and grading, *do not* use anything else like \n    #     np.random.choice() for performing search control.\n\n    # ----------------\n    # your code here\n    # Repeat for the required number of planning steps\n    for _ in range(self.planning_steps):\n        # Choose a state from the set of states in the model\n        state = self.planning_rand_generator.choice(list(self.model.keys()))\n        # Choose an action from the set of actions taken in that state\n        action = self.planning_rand_generator.choice(list(self.model[state].keys()))\n        # Query the model for the predicted next state and reward\n        next_state, reward = self.model[state][action]\n        # Add the bonus to the reward\n        bonus = self.kappa * np.sqrt(self.tau[state, action])\n        adjusted_reward = reward + bonus\n        # Compute the target for updating Q-values\n        if next_state == -1:\n            # If next_state is terminal, target is just the adjusted reward\n            target = adjusted_reward\n        else:\n            # Use the Q-learning update rule with the adjusted reward\n            target = adjusted_reward + self.gamma * np.max(self.q_values[next_state, :])\n        # Update the Q-value for the state-action pair\n        self.q_values[state, action] += self.step_size * (target - self.q_values[state, action])\n    \n    # ----------------')
