...
# your code here
# Move Up from Start State to Top Row
for x in range(3, 0, -1):  # From row 3 to row 1
    state = x * env.grid_w + 0  # Column 0
    policy[state] = [1, 0, 0, 0]  # Up
    
# Move Right along the Top Row
for y in range(0, 11):  # Columns 0 to 10
    state = 0 * env.grid_w + y
    policy[state] = [0, 0, 0, 1]  # Right

# Move Down to the Goal
for x in range(0, 3):  # Rows 0 to 2
    state = x * env.grid_w + 11  # Column 11
    policy[state] = [0, 0, 1, 0]  # Down
